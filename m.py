...
def merge_grids(grids):
    ...
    for row_index in range(0, len(grids), 2):
        ...
        for col_index in range(0, len(row), 2):
            merged_cells = []
            for subgrid in grids[row_index:row_index+2]:
                merged_cells.extend(subgrid[col_index:col_index+2])
            merged_row.append(merged_cells)
...
def generate_results(iteration, grids):
    ...
    for row_index, row in enumerate(grids):
        for col_index, grid in enumerate(row):
            ...
            for node in grid:
                print(f" Nodes: ({node.x}, {node.y})", end="")
            print()
