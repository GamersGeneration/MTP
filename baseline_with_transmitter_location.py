...
def main():
    ...
    min_transmitters_required = len(all_powered_nodes)-1
    print(f"\nMinimum number of transmitters required: {min_transmitters_required}")

    # Step 5: Print locations or node numbers where transmitters are to be placed
    transmitters_locations = set()

    for node_id, powered_at_each_angle in powered_nodes.items():
        for _, powered_set in powered_at_each_angle:
            transmitters_locations.update(powered_set)

    print(f"\nTransmitters should be placed at nodes: {transmitters_locations}")
